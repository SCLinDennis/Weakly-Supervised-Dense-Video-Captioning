...
def pretrained_model(img_shape, length):
    ...
    #x =  Convolution2D(2048, kernel_size = (3, 3), padding = "valid", strides=(2, 2), activation = 'relu', name='block6_pool')(x)
    features = Dense(length, activation = 'sigmoid', use_bias = True,  kernel_initializer='glorot_normal',  bias_initializer='zeros', input_shape = (None, 4, 4, 2048), name='fc7')(x)
    #features = Dropout(0.9, name ='Drop_7')(x)
    def fcn_layer(instance_p):        
        #instance_p = K.reshape(features, [-1, 16, length])
        tmp = K.sum(K.sum(K.log(1-instance_p), axis = 1), axis = 1)
        pwi = 1. - K.exp(tmp)
        return pwi    
...
def label_preprocess(sentences, wordtoix):
    label = [0.0]*len(wordtoix.keys())
    for sentence in sentences:
        words = sentence.split()
        for word in words:
            if word in wordtoix:
                label[wordtoix[word]] = 1.0    
    return label
...
#    checkpoint = ModelCheckpoint(filepath, monitor='acc', verbose=1, save_best_only=True, save_weightmode='max')
