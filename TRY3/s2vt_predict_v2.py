...
batch_size = 32
featype = "IV3"
data_path = '../MSRVTT_hw2_data/'



ixtoword = pd.Series(np.load('./ixtoword_pyf.npy').tolist())

bias_init_vector = np.load('./bias_init_vector_pyf.npy')
...
saver.restore(sess, "./models/pyf/model2158-690")
print("restore success")

test_folder_path = data_path+"testing_data_" + featype + "/feat/"
test = json.load(open(data_path+"testing_label.json",'r'))
...
for test_tuple in test_features:
    video_feat = test_feature_dict[test_tuple[0]]
    video_feat = video_feat.reshape(1,30,2048)
...
#    print (generated_sentence,'\n')
    test_sentences[test_tuple[0]] = generated_sentence
...
for item in test:
    ...
    score_per_video.append(bl.BLEU(test_sentences[item['id']],captions, 4, True))
    bleu.append(score_per_video[0])
    #print(len(score_per_video))
